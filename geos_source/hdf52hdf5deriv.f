...
* See license.md or http://gitlab.com/loic.cg.rossi/pymiedap for details.
      ...
      SUBROUTINE hdf52hdf5deriv(foufile,rfou,outputname)
      USE HDF5
...
*----------------------------------------------------------------------------
      ...
      INCLUDE 'max_incl'
      INTEGER nfou,nmat,nmu,i,m,k
      INTEGER, DIMENSION(:), ALLOCATABLE :: ki
      DOUBLE PRECISION xmu(nmuMAX),
     .                        rfou(nmuMAX*nmatMAX,nmuMAX,0:nfouMAX)
      REAL*8  eps1
      DOUBLE PRECISION, DIMENSION(:), ALLOCATABLE :: xmu_out
      DOUBLE PRECISION, DIMENSION(:,:,:), ALLOCATABLE :: rfou_out,PDD
...
! Derivs array
      INTEGER(HSIZE_T), DIMENSION(5) :: dims4
      INTEGER(HSIZE_T), DIMENSION(5) :: data_dims4
      REAL*8, DIMENSION(:,:,:,:,:),ALLOCATABLE :: dset_data4
...
! =====================================================================
     ...
     .                  error)
      ...
      xmu(:nmu)=xmu_out
      DEALLOCATE(xmu_out)
      ALLOCATE(rfou_out(nmu*nmat,nmu,0:nfou-1))
     ...
     .                  error)
      ...
      rfou(:nmat*nmu,:nmu,0:nfou-1)=rfou_out
      DEALLOCATE(rfou_out)
...
*********************************************
      ...
      dims3(3)=nfou
      rank = 3
      ALLOCATE(dset_data3(nmu*nmat,nmu,0:nfou-1))
      dset_data3 = rfou
! Create dataspace 1
...
*********************************************
      ...
      WHERE (abs(rfou).LT.eps1)
         rfou=0.D0
      ENDWHERE
      ALLOCATE(ki(nmu),PDD(3,nmu,nmu),
     .                  dset_data4(0:nfou-1,nmat,3,nmu,nmu))
      DO m=0,nfou
         DO k=1,nmat
            ki=((/(i,i=1,nmu,1)/)-1)*nmat+k
            call RGPD3P(xmu,xmu,rfou(ki,:,m),PDD,nmu,nmu)
            dset_data4(m,k,:,:,:)=PDD
         ENDDO
      ENDDO
      DEALLOCATE(ki,PDD)
...
! Create dataset 1 with default properties
      CALL h5dcreate_f(file_id, dsetname4, H5T_NATIVE_DOUBLE,
     .            dspace4_id,dset4_id, error)
! Write dataset 1
      CALL h5dwrite_f(dset4_id, H5T_NATIVE_DOUBLE, dset_data4,
     .            data_dims4,error)
! Close access to dataset 1
