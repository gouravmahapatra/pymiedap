...
C   ANY DAMAGES THAT MAY RESULT FROM THE USE OF THE PROGRAM. 
     ...
     &       BET1(NPL1),BET2(NPL1)
      REAL*8 R(2),XI(2),N(2),K(2),LAM
      OPEN (1, FILE='bisphere.print')
C
C INPUT PARAMETERS
C
      LAM=DACOS(-1D0)*2D0
...
C
      WRITE (1,9000) R(1),R(2),R12,LAM
 9000 FORMAT ('R1=',D10.4,'  R2=',D10.4,'  R12=',D10.4,'  LAM=',D10.4)
      IF (R12.LT.R(1)+R(2)) THEN 
         WRITE(*,'(''R12 IS SMALLER THAN R(1)+R(2)'')')      
         STOP     
      ENDIF
      IF (R(2).GT.R(1)) THEN 
         WRITE(*,'(''R(2) IS GREATER THAN R(1)'')')      
         STOP     
      ENDIF
      PI=DACOS(-1D0)
      XI(1)=2D0*PI*R(1)/LAM
     ...
     &               CEXT,CSCA,NODR1,NODRT1,L1MAX,LAM)
      WALB=CSCA/CEXT
...
C     WRITE (1,9100) QEXT,QSCA
 9100 FORMAT ('QEXT=',D12.6,'  QSCA=',D12.6)
C
...
C
    ...
    5 CONTINUE
      WRITE (6,1000) FAC(165)
 1000 FORMAT ('FAC(165)=',D10.4)
C
      NOPT=0
      DO I=1,2
         NODR(I)=NINT(XI(I)+4.*XI(I)**(1./3.)+NODR1)
         IF(NODR(I).GT.NOD) THEN
            WRITE(*,'(''NOD DIMENSION EXCEEDED!'')')      
            STOP     
         ENDIF
C
...
C
     ...
     1             QS1(I),AN1(1,I),CN1(1,I),0)
      ...
      WRITE (1,9500) X12,NODRT
 9500 FORMAT ('X12=',D10.4,'  NODRT=',I3)
      IF(NODRT.GT.NOTD) THEN
         WRITE(*,'('' NOTD DIMENSION EXCEEDED!'')')
         STOP    
      ENDIF
...
C CALCULATE THE EXPANSION COEFFICIENTS
C
      WRITE (1,9000) XI(1),SN(1),SK(1),NODR(1)
      WRITE (1,9001) XI(2),SN(2),SK(2),NODR(2)
 9000 FORMAT('X1=',D12.6,' N1=',D12.6,' K1=',D12.6,' NODR(1)=',I3)
 9001 format('X2=',D12.6,' N2=',D12.6,' K2=',D12.6,' NODR(2)=',I3)
      DLAM=2d0*PI
      ...
      WALB=CSCA/CEXT
      ASYMM=AL1(2)/3D0
      write (1,9100) CEXT,CSCA,WALB,ASYMM
      XI1=XI(1)
      XI2=XI(2)
...
C     WRITE (1,9200) QEXT,QSCA,QABS
 ...
 9200 FORMAT(' QEXT=',D12.6,2X,'QSCA=',D12.6,2X,'QABS=',D22.16)
      RETURN
      END
...
C        180*(I-1)/(NPNA-1) (DEGREES), WHERE I NUMBERS THE ANGLES
 ...
 1001 FORMAT(2X,'S',6X,'ALPHA1',6X,'ALPHA2',6X,'ALPHA3',
     &       6X,'ALPHA4',7X,'BETA1',7X,'BETA2')
      DO 1 L=1,L1MAX
         WRITE (1,1002) L-1,A1(L),A2(L),A3(L),A4(L),B1(L),B2(l)
    1 CONTINUE
      WRITE (1,1000)
 1002 FORMAT (I3,6F12.5)
      ...
      D6=DSQRT(6D0)*0.25D0
      WRITE (1,1003)
 1003 FORMAT(' ',5X,'<',7X,'F11',7X,'F22',7X,'F33',
     & 7X,'F44',7X,'F12',7X,'F34')
      DO 500 I1=1,N
         TAA=TAA+DA
         TB=TB+DB
         U=DCOS(TAA)
...
C        F12=-100D0*F12/F11
C        F34=100D0*F34/F11
         WRITE (1,1004) TB,F11,F22,F33,F44,F12,F34
  500 CONTINUE
 1004 FORMAT(' ',F6.2,6F10.5)
      RETURN
      END
...
c****************************************************************
      ...
      DO 100 L=1,L1
         ...
         DO 50 I=1,11
            ...
            IF((DL+C2)*(DL-C3)-C1.LE.-1D-6) KONTR=2
            IF((DL-C2)*(DL+C3)-C1.LE.-1D-6) KONTR=2
            IF(KONTR.EQ.2) WRITE (1,4000) LL,C
   50    CONTINUE
  100 CONTINUE
      IF(KONTR.EQ.1) WRITE (1,2000)   
...
C********************************************************************
     ...
     &               ALF1,ALF2,ALF3,ALF4,BET1,BET2,LMAX)
