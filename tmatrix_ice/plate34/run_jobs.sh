#!/bin/bash
cd "$HOME/build"; JOBS=$1; BUD=$2; START=$SECONDS
while :; do
  line=$(head -1 "$JOBS" 2>/dev/null); [ -z "$line" ] && { echo "ALL DONE $JOBS"; break; }
  ...
  [ $((SECONDS-START)) -ge $BUD ] && { echo "paused: $(wc -l < $JOBS) left in $JOBS"; break; }
done
