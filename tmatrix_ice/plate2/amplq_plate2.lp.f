...
C  PLATE 2: degree of linear polarization -Z21/Z11 vs scattering angle,
C  for a spheroid in FIXED orientation. IPANEL=2 -> axis PARALLEL to incident
C  beam (panel b); otherwise axis PERPENDICULAR (panel c).
      IF (IPANEL.EQ.2) THEN
         BETA=0D0
      ELSE
         BETA=90D0
      ...
      PHI=0D0
      ALPHA=0D0
      OPEN (7,FILE='ampl.out')
      DO 999 IANG=0,180
         THET=DFLOAT(IANG)
         TT0=THET0
         TT=THET
C        nudge away from exact pole/forward/back to avoid azimuth singularity
         IF (TT0.LT.1D-4) TT0=1D-4
         IF (TT.LT.1D-4) TT=1D-4
         IF (TT.GT.180D0-1D-4) TT=180D0-1D-4
         CALL AMPL (NMAX,DLAM,TT0,TT,PHI0,PHI,ALPHA,BETA,
     &              S11,S12,S21,S22)
     ...
     &             -S21*DCONJG(S21)-S22*DCONJG(S22))
         DLP=-Z21/Z11
         WRITE (7,7777) THET, Z11, Z21, DLP
  999 CONTINUE
      CLOSE (7)
 7777 FORMAT (F8.2,3E16.7)
...
c         PRINT 5000, CHECK
   ...
   20         PC=X*PB+(X*PB-PA)*(DJ-A)/DJ
          ...
          W(I)=W(M)
  100 CONTINUE
 5000 FORMAT ('QGAUSS DOES NOT CONVERGE, CHECK=',E10.3)
...
C**********************************************************************
 ...
 9000 FORMAT(' NMAX = ',I2,', i.e., greater than ',I3)
      ...
      TB=MAX(TB,DFLOAT(NMAX))
      NNMAX1=8.0Q0*SQRT(MAX(TA,DFLOAT(NMAX)))+3Q0
...
C**********************************************************************
      ...
      L=NMAX+NNMAX
      XX=1Q0/X
      Z(L)=1Q0/(DFLOAT(2*L+1)*XX)
      L1=L-1
      DO 5 I=1,L1
         I1=L-I
         Z(I1)=1Q0/(DFLOAT(2*I1+1)*XX-Z(I1+1))
    5 CONTINUE
      Z0=1Q0/(XX-Z(1))
      Y0=Z0*COS(X)*XX
      Y1=Y0*Z(1)
...
C**********************************************************************
      ...
      REAL*16 CYR(NPN1),CYI(NPN1),CZR(5000),CZI(5000),
     *        CUR(NPN1),CUI(NPN1)
      L=NMAX+NNMAX
...
c     0.LE.x.LE.1
   ...
   20 QMM=DFLOAT(M*M)
      DO 25 I=1,M
         I2=I*2
         A=A*SQRT(DFLOAT(I2-1)/DFLOAT(I2))*QS
   25 CONTINUE
      D1=0Q0
      D2=A 
      DO 30 N=M,NMAX
