...
def integration(body):
	...
	import pymiedap.module_geos as geos
	ngeosMAX=200000
	...
	scene = body.properties.fourier_scene
	    ...
	    ######################################################################################
	    											##
            I,Q,U,V = pmd.read_dap_output(np.repeat(phase[i], sum(A)), sza[i,A], emission[A],  
				'exopy/files/'+ files[scene], beta=beta[i,A], phi=phi[i,A])     ##
	    											##
	    ######################################################################################
	    ##########################       GEOS.READ_DAP      ##################################
	    ######################################################################################
